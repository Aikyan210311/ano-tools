...
- 最新本地验证：`npm.cmd run lint`、`npm.cmd run build`、本地 `http://127.0.0.1:5178` 均通过。
## 2026-04-28 新番资讯页面计划更新
- 新增新番资讯页面第一版：使用 `#anime` hash 路由，静态样例数据，支持星期切换、搜索、平台/来源筛选、只看今天、详情弹窗和本地追番状态。
- 页面暂不接真实 API；后续可以把 `src/data/anime.ts` 替换为真实季度数据，或接 Bangumi/AniList 等 API。
- 根据用户反馈重新改为“选中星期后，下方整页展示当天番剧卡片”的周历模式；卡片 80% 用于封面，20% 用于番名，只保留更新时间和集数角标。
- 已从 `https://yuc.wiki/202604/` 预置 2026 年 4 月新番周历数据 69 条；详情页保留平台、开播、制作等后续可补充信息。
