...
### 2026-04-28：新番资讯页面第一版
- **状态：** 本地完成，尚未发布线上。
- **执行的操作：**
  - 新增 `src/data/anime.ts`，保存季度信息和新番样例数据。
  - 将首页接入 `#anime` hash 路由，NEWS 和右上菜单“新番资讯”可进入新页面。
  - 新增新番资讯页：季度标题、今日统计、星期切换、搜索、平台筛选、来源筛选、只看今天、我的追番和详情弹窗。
  - 追番状态使用浏览器 `localStorage` 本地保存。
  - 新增桌面和移动端响应式样式。
- **验证结果：**
  - `npm.cmd run lint` 通过。
  - `npm.cmd run build` 通过。
  - 本地开发服务 `http://127.0.0.1:5180/#anime` 返回 HTTP 200。
### 2026-04-28：新番资讯页精简为周历和追番
- **状态：** 本地完成，尚未发布线上。
- **执行的操作：**
  - 移除新番页顶部“今日/关注”统计块。
  - 移除搜索、平台筛选、来源筛选、只看今天面板。
  - 重排页面为“周历排播 + 我的追番”两段式布局。
  - 强化星期条、7 日周历列、番剧卡片和追番面板的视觉细节。
- **验证结果：**
  - `npm.cmd run lint` 通过。
  - `npm.cmd run build` 通过。
  - 本地开发服务 `http://127.0.0.1:5180/#anime` 返回 HTTP 200。
### 2026-04-28：新番周历改为选中日卡片网格
- **状态：** 本地完成，尚未发布线上。
- **执行的操作：**
  - 从 `https://yuc.wiki/202604/` 本地页面缓存解析周一到周日排播，生成 69 条 2026 年 4 月新番预置数据。
  - 将新番页从 7 列整周日历改成“顶部星期选择条 + 下方选中日番剧网格”。
  - 将番剧卡片改成封面主导布局：80% 封面、20% 番名，左上角显示更新时间，右下角显示按开播日期动态计算的集数进度。
  - 列表卡片移除平台、来源、标签、日文名等信息；这些内容保留在详情弹窗中。
  - 详情弹窗改为展示真实封面，并增加数据源入口。
- **验证结果：**
  - `npm.cmd run lint` 通过。
  - `npm.cmd run build` 通过。
  - 本地开发服务 `http://127.0.0.1:5180/#anime` 返回 HTTP 200。
